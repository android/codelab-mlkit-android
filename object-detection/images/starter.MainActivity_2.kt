...
class MainActivity : AppCompatActivity(),
    ...
    private fun getCapturedImage(): Bitmap {
        ...
        // retrieve captured image and fix up rotation
        val stream: InputStream? = baseContext.getContentResolver()
            .openInputStream(this.outputFileUri)
        ...
        var srcImage = MediaStore.Images.Media.getBitmap(
            getContentResolver(), outputFileUri
        )
        if (rotationInDegrees != 0) {
            val tmpImage = srcImage
            val matrix = Matrix()
            matrix.preRotate(rotationInDegrees.toFloat())
            srcImage = Bitmap.createBitmap(
                tmpImage, 0, 0,
                tmpImage.width, tmpImage.height, matrix, false
            )
            tmpImage.recycle()
        }
